...
export function PropertyCard({ property }: { property: Property }) {
  ...
  return <article className="group animate-rise">
    <div className="relative aspect-[1.18/1] overflow-hidden rounded-[14px] bg-stone-200 dark:bg-stone-800">
      ...
      <div className="absolute inset-x-0 bottom-3 flex justify-center gap-1.5">{property.images.map((_, index) => <button key={index} onClick={() => setImageIndex(index)} aria-label={`Show photo ${index + 1}`} className={`h-1.5 rounded-full shadow ${index === imageIndex ? "w-4 bg-white" : "w-1.5 bg-white/70"}`} />)}</div>
