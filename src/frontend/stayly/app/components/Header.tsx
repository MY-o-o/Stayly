...
export function Header() {
  ...
  return (
    <header
      className={`sticky top-0 z-40 border-b border-stone-100 transition-all dark:border-white/10 ${
        isScrolled
          ? "bg-white/90 shadow-sm backdrop-blur-xl dark:bg-[#0f0f0f]/90"
    ...
    >
      <div className="mx-auto flex max-w-[1560px] items-center justify-between gap-5 px-5 py-4 lg:px-10">
        <Link
          href="/"
          className="group flex items-center gap-2 text-2xl font-extrabold tracking-[-1.2px] text-[#ff385c]"
          aria-label="Stayly home"
        >
          <span className="grid size-8 place-items-center rounded-xl bg-[#ff385c] text-base text-white transition-transform group-hover:rotate-6">
            S
          </span>
          Stayly
        </Link>
        ...
        <nav className="hidden items-center gap-6 text-sm font-medium xl:flex" aria-label="Primary navigation">
          <Link className="hover:text-[#ff385c] transition-colors" href="/#stays">
            Stays
          </Link>
          <a className="hover:text-[#ff385c] transition-colors" href="/#experiences">
            Experiences
          </a>
          <a className="hover:text-[#ff385c] transition-colors" href="/#online">
            Online Experiences
          </a>
        </nav>
        <div className="flex items-center gap-2">
          {isAuthenticated && user && (
            <span className="hidden text-sm font-semibold text-stone-700 dark:text-stone-200 sm:inline">
              Hi, {user.name}
            </span>
          )}
          ...
          <div className="relative" ref={menuRef}>
            ...
            >
              <Icon name="menu" className="size-4 text-stone-700 dark:text-stone-200" />
            ...
            {menuOpen && (
              <div className="absolute right-0 top-13 z-50 w-64 origin-top-right rounded-2xl border border-stone-100 bg-white py-2 shadow-xl ring-1 ring-black/5 transition-all dark:border-white/10 dark:bg-[#1c1c1c] dark:shadow-2xl animate-rise">
                {isAuthenticated && user ? (
                  <>
